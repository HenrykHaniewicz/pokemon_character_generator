...
def train_conditional_generator_with_quality(model, dataloader, z_dim, device, epochs, lambda_outline=0.5, lambda_perceptual=1.0,
    ...
    perceptual_losses, outline_losses, quality_losses, total_losses = [], [], [], []
    ...
    for epoch in range(epochs):
        ...
        for real_imgs, meta_vecs, quality in dataloader:
            ...
            meta_vecs = meta_vecs.to(device)
            quality = quality.float().to(device).unsqueeze(1) / 10.0
        ...
        print(f"[Epoch {epoch+1}] Loss: {avg_loss:.4f} (Perceptual: {perceptual_loss.item():.4f}, Outline: {outline_loss.item():.4f}, Quality: {quality_loss.item():.4f})")
    ...
    plt.savefig("loss_plot_graded.png"); plt.close()
